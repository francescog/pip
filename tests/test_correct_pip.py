...
def test_correct_pip_version():
    ...
    import re,filecmp
    pip_dir = re.match(r'\s*pip\s\S+\sfrom\s+(.*)\s\([^(]+\)$', result.stdout.replace('\r\n','\n')).group(1)
    ...
    diffs = filecmp.dircmp(join(base,'pip'), join(pip_dir,'pip'))
    ...
    assert not mismatch_py, 'mismatched source files in %r and %r'% (join(base,'pip'), join(pip_dir,'pip'))
